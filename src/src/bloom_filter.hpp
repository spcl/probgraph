...
public:
   ...
   float approximate_intersection_count(const bloom_filter& f) const
   { 
      float ret = 0;
      int nbits = 0;
      if (
         ...
         )
      {
         ...
         for (std::size_t i = 0; i < bit_table_.size(); ++i)
         {
            tmp = bit_table_[i] & f.bit_table_[i];
            nbits += bits_set(tmp);
         }
         ret = -((float)table_size_/salt_count_)*log(1-(float)nbits/table_size_);
      }
      return ret; 
   }
   int bits_set(unsigned char v) const
   {
       static const unsigned char  S[]={1,2,4};
       static const unsigned char  B[]={0x55,0x33,0x0F};
       unsigned char c = v-((v>>1)&B[0]);
       c = ((c >> S[1]) & B[1]) + (c & B[1]);
       c = ((c >> S[2]) + c) & B[2];
       return c;
   }

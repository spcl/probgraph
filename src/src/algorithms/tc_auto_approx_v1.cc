...
#pragma omp parallel for schedule(dynamic, 64)
  for (NodeID u = 0; u < g.num_nodes(); u++) {
    ...
    for (NodeID m = 0; m < messages[u].size(); m++) {
      for (NodeID v : g.out_neigh(u)) {
...
#pragma omp parallel for reduction(+:total) schedule(dynamic, 64)
  ...
  std::cout << "RRR TC BSP1 TC_BSP1 " << graphName << " " << threads << " " << g.num_nodes() << " " << g.num_edges() << " " << threshold << " " << k << " "  << m << " " << pp_time << " " << tc_time << " " << pp_time + tc_time <<  " " << total << std::endl;
  ...
  std::cout << "SSS TC BSP1 TC_BSP1 " << graphName << " " << threads << " " << g.num_nodes() << " " << g.num_edges() << " " << threshold << " " << k << " " << m << " " << approx_str_size << " " << initial_csr_size << " " << approx_str_size + initial_csr_size << " " << total << std::endl;

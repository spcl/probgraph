...
int TCColourful(const Graph &g, float  p_, std::string graphName, int threads)
{
...
#pragma omp parallel for reduction(+ : total) schedule(dynamic, 64)
    ...
    std::cout << "RRR TC COLORFUL TC_COLORFUL " << graphName << " " << threads << " " << g.num_nodes() << " " << g.num_edges() << " " << threshold << " " << k << " "  << m << " " << pp_time << " " << tc_time << " " << pp_time + tc_time <<  " " << total << std::endl;
    ...
    std::cout << "SSS TC COLORFUL TC_COLORFUL " << graphName << " " << threads << " " << g.num_nodes() << " " << g.num_edges() << " " << threshold << " " << k << " " << m << " " << approx_str_size << " " << initial_csr_size << " " << approx_str_size + initial_csr_size << " " << total << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    auto ct = [&cli] (const Graph& g){
        return TCColourful(g, cli.p(), cli.getGraphBasename(), cli.getThreadNum());
    };

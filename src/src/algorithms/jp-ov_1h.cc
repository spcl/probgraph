...
int64_t OrderedCount(const Graph &g, float treshold, int k_, float tau, std::string graphName, int threads) {
    ...
    for(NodeID u = 0; u<g.num_nodes(); ++u) {
        ...
        OneHashSet<NodeID> * neigh = new OneHashSet<NodeID>(g.out_neigh(u).begin(), g.out_neigh(u).end(), max_degree * treshold, seed);
        sets[u] = neigh;
    }
    t.Stop();
    ...
    std::cout << "RRR JP-OV 1H JP-OV_1H " << graphName << " " << threads << " " << g.num_nodes() << " " << g.num_edges() << " " << treshold << " " << k << " " << m << " " << tau << " " << pp_time << " " << alg_time << " " << pp_time + alg_time <<  " " << total << std::endl;
    ...
    std::cout << "SSS JP-OV 1H JP-OV_1H " << graphName << " " << threads << " " << g.num_nodes() << " " << g.num_edges() << " " << treshold << " " << k << " " << m << " " << tau << " " << approx_str_size << " " << initial_csr_size << " " << approx_str_size + initial_csr_size << " " << total << std::endl;

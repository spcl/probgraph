...
float jaccardEstimate(KMinHashSet<int> * set_a, KMinHashSet<int> * set_b, size_t k) {  
...
void testCreation() {
  cout << "Testing MinhashSet creation..." << endl;
  // struct timeval time;
  ...
  KMinHashSet<int> * test_set = new KMinHashSet<int>(begin, end, k, seed);
  assert(test_set->seed == seed);
  assert(test_set->size() == 5);
  assert(test_set->k == k);
  assert(test_set->min_count == (int) k);

  // The k/min_count values should be sorted from lowest to highest
  assert(test_set->values[0] == 2);
  assert(test_set->values[1] == 3);
  cout << "Success!" << endl;
  delete test_set;
}
...
void testIntersectionCount(vector<int> &vec_a, vector<int> &vec_b, size_t k) {
  cout << "Testing MinhashSet intersection..." << endl;
  int seed = 649135;
  // The KMinHashSet constructor requires pointers instead of iterators
  int* begin_a = &vec_a[0];
  int* end_a = &vec_a[0] + vec_a.size();
  KMinHashSet<int> * set_a = new KMinHashSet<int>(begin_a, end_a, k, seed);
  int* begin_b = &vec_b[0];
  int* end_b = &vec_b[0] + vec_b.size();
  KMinHashSet<int> * set_b = new KMinHashSet<int>(begin_b, end_b, k, seed);
  // printVector(vec_a);
  // printSet(set_a);
  // printVector(vec_b);
  // printSet(set_b);
...
void testIntersectionCountMultiple() {
  cout << "Testing MinhashSet multiple intersection..." << endl;
  int seed = 649135;
  size_t k = 14;
  ...
  KMinHashSet<int> * set_c = new KMinHashSet<int>(begin_c, end_c, k, seed);
  ...
  // vector<KMinHashSet<int> *> kminsets = {&set_b, &set_c};
  vector<KMinHashSet<int> *> kminsets;
  kminsets.push_back(set_b);
  kminsets.push_back(set_c);
  float actual_estimate = set_a->intersect_count_multiple(kminsets);

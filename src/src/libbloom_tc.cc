...
#include "graph.h"
#include "pvector.h"
#include "lib/bf/bloom_filter.hpp"
#include "lib/bf/bloom_filter/basic.hpp"
...
using namespace std;
typedef bf::basic_bloom_filter bloom_filter;
vector<bloom_filter> generate_filters(double acc, const double scale, const Graph &g){
  ...
  for(NodeID u = 0; u<n; u++){
    filters.push_back(bloom_filter(acc, (size_t) (max_deg * scale)));
    for (auto it = g.out_neigh(u).begin(); it != g.out_neigh(u).end(); it++){
        filters[u].add(*it);
    }
  }
  return filters;
...
int main(int argc, char* argv[]) {	
  ...
  auto filters = generate_filters(cli.accuracy(), cli.scale(), g);

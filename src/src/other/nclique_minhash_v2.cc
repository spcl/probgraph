...
size_t intersect_count(vector<int> &nodes, vector<KMinHashSet<NodeID>*> &sets, size_t *counter=nullptr) {
...
pair<vector<int>, size_t> intersect_intermediate(vector<int> &nodes, vector<KMinHashSet<NodeID>*> &sets, size_t *counter=nullptr) {
  vector<KMinHashSet<NodeID>*> isect_sets;
  isect_sets.reserve(nodes.size()); 
  for (auto node : nodes) {
    isect_sets.push_back(sets[node]);

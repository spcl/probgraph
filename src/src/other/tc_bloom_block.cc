...
std::vector<NodeID*> addSets(vector<BlockBloomSet<NodeID>*> &sets,const Graph &g,int block_size, float k_scale){
  ...
  //}
  int m = (int)(k_scale*block_size);
  int k = (float)m/block_size*0.69324718056;
  if(m<32) m=32;
  if(k<1)k=1;
  struct timeval seed;
  ...
  for (NodeID u = 0; u < g.num_nodes(); ++u) {
    ...
    BlockBloomSet<NodeID> * neigh = new BlockBloomSet<NodeID>(g.out_neigh(u).begin(), vcandidates_start,k, m, block_size, seeds);
...
size_t OrderedCount(const Graph &g, int block_size, float k_scale){
    Timer t;
    t.Start();
    vector<BlockBloomSet<NodeID>*> sets;
    vector<NodeID*> hstart;
    hstart = addSets(sets, g, block_size, k_scale);
    t.Stop();
...
int main(int argc, char* argv[]) {
  CLBlockApp cli(argc, argv, "Bloom-block triangle count");
  if (!cli.ParseArgs())
    return -1;
  Builder b(cli);
  Graph g = b.MakeGraph();

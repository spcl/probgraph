...
std::vector<NodeID*> addSets(vector<myBloomSet<NodeID>*> &sets,const Graph &g,float tres){
    int64_t max_degree = 0;
    for(NodeID u =0; u<g.num_nodes(); ++u)
        max_degree = std::max(max_degree, g.out_degree(u));
    int m = max_degree * tres;
    printf("bf size: %d\n", m);
    if (m<max_degree) m = max_degree;
    int k = 1;
  ...
  //for(NodeID u = 0; u<g.num_nodes(); u++){
  //    myBloomSet<NodeID> * neigh = new myBloomSet<NodeID>(g.out_neigh(u).begin(), g.out_neigh(u).end(), m, k);
  ...
  for (NodeID u = 0; u < g.num_nodes(); ++u) {
    ...
    myBloomSet<NodeID> * neigh = new myBloomSet<NodeID>(g.out_neigh(u).begin(), vcandidates_start, m, k, seeds, g.num_nodes());

...
std::vector<NodeID*> addSets(vector<BlockKMVSet<NodeID>*> &sets,const Graph &g,int block_size, float k_scale, int seed){

  //for(NodeID u = 0; u<g.num_nodes(); u++){
  //    BlockKMVSet<NodeID> * neigh = new BlockKMVSet<NodeID>(g.out_neigh(u).begin(), g.out_neigh(u).end(), k_scale, block_size, seed);
  ...
  for (NodeID u = 0; u < g.num_nodes(); ++u) {
    ...
    BlockKMVSet<NodeID> * neigh = new BlockKMVSet<NodeID>(g.out_neigh(u).begin(), vcandidates_start,k_scale, block_size, seed);
...
size_t OrderedCount(const Graph &g, int block_size, float k_scale) {
  ...
  Timer t;
  t.Start();
  vector<BlockKMVSet<NodeID>*> sets;
...
int main(int argc, char* argv[]) {
  CLBlockApp cli(argc, argv, "KMV-block triangle count");
  if (!cli.ParseArgs())
    return -1;
  Builder b(cli);
  Graph g = b.MakeGraph();

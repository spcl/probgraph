...
vector<bloom_filter> generate_filters(const double acc, const Graph &g){
  ...
  for(NodeID u = 0; u<n; u++){
    ...
    param.projected_element_count=g.out_degree(u);
    param.false_positive_probability=acc;
    param.random_seed=random_device{}();
...
size_t OrderedCount(const double acc, const Graph &g, vector<bloom_filter> &filters) {
  size_t total = 0;
      ...
      if (v > u) 
        break;
          for (NodeID w : g.out_neigh(v)) {
            if (w > v)
                break;
            if(g.out_degree(w) > g.out_degree(u) ? filters[w].contains(u) : filters[u].contains(w)){
                total = total + 1;
            }
          }
